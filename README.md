...
- **캠핑장 정보:** GoCamping 기반 캠핑장 데이터를 MongoDB에서 조회해 지역·취향 조건에 맞는 정보를 제공했습니다.
...
저장소의 `kochat/`와 `docs/01~07`은 KoChat 기반 코드와 관련 원본·참고 문서이며, CAMPSTER 적용 코드 및 서버 종료 이후 구성한 보존용 demo와 구분됩니다.

## 담당 역할

Frontend는 팀원 3명이 함께 담당했으며, 화면 설계와 서비스 흐름을 논의하고 모바일 채팅 UI를 공동 구현했습니다. Frontend 공동 구현 범위는 다음과 같습니다.
...
수업 종료 후 NHN Cloud 서버와 기존 실행 환경을 더 이상 유지하지 않아 원본 시스템을 그대로 실행하기 어려워졌습니다. 포트폴리오 보존을 위해 별도의 demo를 구성했으며, `portfolio` 브랜치에서 시작한 정리 결과를 현재 `main`에도 반영했습니다.
...
- 현재 공개 트리에서 과거 서버 접속 설정을 제거하고, API 키는 환경변수 기반으로 관리
